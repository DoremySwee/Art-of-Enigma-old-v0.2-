...
for i in ae2.items{
    ...
    if(i.definition.id.contains("seed")){
        continue;}
    if(i.definition.id.contains("_quartz_")){
        continue;}
    if(i.definition.id.contains("paint_ball")){
        continue;}
    if(i.definition.id.contains("pattern")){
        continue;}
    if(i.definition.id.contains("crank")){
        continue;}
    if(i.definition.id.contains("grindstone")){
        continue;}
    if(i.ores.length<1){
        c+=1;
        recipes.remove(i);
